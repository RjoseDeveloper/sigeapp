...
class PautaFrequencia {
    ...
    {
        $query = "SELECT DISTINCT (SUM(estudante_nota.nota)/COUNT(data_avaliacao.id_data)) * planoavaliacao.peso/100  as media, planoavaliacao.peso,
                data_avaliacao.idplano, data_avaliacao.idtipo_avaliacao from estudante_nota
                ...
                WHERE pautanormal.idDisciplina = '$disciplina' AND estudante_nota.idaluno = '$aluno'
                AND pautanormal.estado= '$estado' AND pautanormal.idcurso= '$curso' AND data_avaliacao.status='$estado'
                GROUP BY data_avaliacao.idplano, data_avaliacao.idtipo_avaliacao";
    ...
    public function getNotaExame($disciplina, $aluno, $estado,$curso, $ctr)
    {
        ...
        $query = "SELECT DISTINCT  estudante_nota.nota,data_avaliacao.descricaoteste as descricao from estudante_nota
    ...
    public function ordenacaoTestes($disciplina, $aluno, $estado,$curso, $ctr)
    {
        ...
        INNER JOIN pautanormal on pautanormal.idPautaNormal = estudante_nota.idPautaNormal
        INNER JOIN data_avaliacao  ON data_avaliacao.id_data = pautanormal.idTipoAvaliacao
        WHERE  pautanormal.idDisciplina= '$disciplina' AND pautanormal.idcurso = '$curso' AND 
        pautanormal.estado= '$estado' AND data_avaliacao.idtipo_avaliacao < 4";
