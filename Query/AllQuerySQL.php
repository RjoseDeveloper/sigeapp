...
class QuerySql{
  ...
  pautanormal.dataReg, pautanormal.dataPub, disciplina.descricao as disp, pautanormal.estado, data_avaliacao.id_data
FROM pautanormal
...
WHERE  data_avaliacao.status='$status' AND pautanormal.idusers ='$id'";
    ...
    public function listaDisciplina($docente, $ctr){
        ...
        $query = "SELECT DISTINCT curso.idcurso,curso.descricao as curso,
disciplina.idDisciplina, disciplina.descricao, disciplina.natureza
FROM disciplina INNER JOIN docente_disciplina ON disciplina.idDisciplina = docente_disciplina.iddisciplina
 INNER JOIN curso ON curso.idcurso = docente_disciplina.idcurso
 INNER JOIN utilizador ON utilizador.id = docente_disciplina.idutilizador
...
WHERE professor.idprofessor = '$docente'";
    ...
    public function discplinasCurso($idcurso)
    {
        return ("SELECT DISTINCT disciplina.idDisciplina as disp, disciplina.descricao FROM disciplina
                 INNER JOIN docente_disciplina ON disciplina.idDisciplina = docente_disciplina.iddisciplina
    ...
    function getCursoProfessor($idDoc){
        return 'SELECT curso.idcurso, descricao from curso
                  INNER JOIN docente_disciplina ON curso.idcurso = docente_disciplina.idcurso
                  INNER JOIN utilizador ON utilizador.id = docente_disciplina.idutilizador
...
WHERE professor.idprofessor='$docente'";
 ...
 AND docentedisciplina.idCurso = '$cr'";
    ...
    public function listaDispCursoDocente($curso, $docente){
		...
		disciplina.descricao as nomeD, disciplina.natureza from disciplina
INNER JOIN docente_disciplina ON disciplina.idDisciplina = docente_disciplina.iddisciplina
INNER JOIN curso ON curso.idcurso = docente_disciplina.idcurso
INNER JOIN utilizador ON utilizador.id = docente_disciplina.idutilizador
INNER JOIN professor ON utilizador.id = professor.idutilizador

      WHERE docente_disciplina.idcurso = '$curso' and professor.idprofessor ='$docente'";
        $result = mysqli_query($db->openConection(),$query);
    ...
    public function getDocenteIdCurso($disciplina, $idDocente)
    {
        ...
        $query = "SELECT DISTINCT curso.idcurso as idc FROM docente_disciplina
                  INNER JOIN  utilizador ON utilizador.id = docente_disciplina.idutilizador
                  INNER JOIN professor ON utilizador.id = professor.idutilizador


                 WHERE docente_disciplina.iddisciplina= '$disciplina'
    ...
    public function queryAutoComplete($keyword, $curso, $disp){
        ...
        return ("SELECT DISTINCT utilizador.codigo as numero, utilizador.fullname  FROM utilizador INNER JOIN inscricao ON utilizador.id = inscricao.idutilizador
...
AND utilizador.nomeCompleto LIKE '$keyword'");
    ...
    public function getDisciplinaDocenteIdCurso($disp, $idDosc){
        $db = new mySQLConnection();
        $sql = "SELECT DISTINCT docente_disciplina.idcurso FROM docente_disciplina
                INNER JOIN utilizador ON utilizador.id = docente_disciplina.idutilizador
                INNER JOIN professor ON utilizador.id = professor.idutilizador
WHERE docente_disciplina.iddisciplina = '$disp' AND professor.idprofessor= '$idDosc'";
    ...
    {
        return ("SELECT DISTINCT professor.idprofessor,utilizador.nomeCompleto
    FROM professor INNER JOIN docente_disciplina
    ON docente_disciplina.idutilizador = professor.idutilizador INNER JOIN disciplina
    ON disciplina.idDisciplina = docente_disciplina.iddisciplina INNER JOIN utilizador
...
ON docente.idUtilizador = utilizador.id
    ...
    {
        $query="  SELECT COUNT(docente_disciplina.iddisciplina) as conta, docente_disciplina.iddisciplina  FROM docente_disciplina
          WHERE iddisciplina = '$idDisp' AND docente_disciplina.idutilizador = '$doc'";
...
WHERE estudante_nota.idNota = '$idnota'";
    ...
    public function datalhes_disciplina($disp, $doc, mySQLConnection $db)
    {
		...
		WHERE docente_disciplina.iddisciplina = '$disp'
