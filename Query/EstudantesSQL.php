...
WHERE inscricao.idutilizador = '$id';";
    ...
    public function qtdAvRealizada($disciplina,$estado,$curso, $ctr){
        $db = new mySQLConnection();
        ...
        $query = "SELECT  data_avaliacao.id_data as tipo, disciplina.idDisciplina,data_avaliacao.descricaoteste as descricao
                    ...
                    ON disciplina.idDisciplina = pautanormal.idDisciplina
                    WHERE disciplina.idDisciplina = '$disciplina' AND data_avaliacao.status= '$estado' and pautanormal.idcurso ='$curso'
...
FROM aluno INNER JOIN utilizador ON utilizador.id = aluno.idutilizador
 ...
 WHERE pautanormal.idPautaNormal = '$ptn' AND pautanormal.estado= '$estado'");
    ...
    {
        return ("SELECT DISTINCT professor.idprofessor as idDocente, utilizador.sexo, professor.idgrau,
utilizador.nomeCompleto FROM professor

            INNER JOIN utilizador ON utilizador.id = professor.idutilizador
           INNER JOIN docente_disciplina ON docente_disciplina.idutilizador = professor.idutilizador

WHERE docente_disciplina.iddisciplina= '$disciplina' AND docente_disciplina.idcurso= '$curso'");
    }
...
AND estudante_nota.nota < 10;";
    ...
    {
        $query ="SELECT  DISTINCT (SUM (estudante_nota.nota)/COUNT(data_avaliacao.id_data))* as media, estudante_nota.idaluno from estudante_nota
